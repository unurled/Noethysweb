...
class Impression(utils_impression.Impression):
    ...
    def Draw(self):
        ...
        questionnaires_individus = utils_questionnaires.ChampsEtReponses(categorie="individu", filtre_reponses=(Q(individu__in=[r.individu for r in rattachements]) & ~Q(reponse__isnull=True)))
        questionnaires_familles = utils_questionnaires.ChampsEtReponses(categorie="famille", filtre_reponses=(Q(famille__in=[r.famille for r in rattachements]) & ~Q(reponse__isnull=True)))
