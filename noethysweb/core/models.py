...
LISTE_METHODES_TARIFS = [
    { "code": "montant_unique", "label":u"Montant unique", "type": "unitaire", "nbre_lignes_max": 1, "entete": None, "champs": ("montant_unique",), "champs_obligatoires": ("montant_unique",), "tarifs_compatibles": ("JOURN", "FORFAIT", "CREDIT") },
...
class AdresseMail(models.Model):
    ...
    moteur = models.CharField(verbose_name="Moteur", max_length=200, choices=[("smtp", "SMTP"), ("mailjet", "Mailjet"), ("brevo", "Brevo"), ("console", "Console")], help_text="Sélectionnez un moteur d'expédition (Smtp, Mailjet ou Brevo).")
...
def chemin_signature(instance, filename):
    import os
    from django.conf import settings
    dossier = os.path.join(settings.MEDIA_ROOT, 'signatures')
...
class Utilisateur(AbstractUser):
    ...
    signature_image = models.FileField(upload_to=chemin_signature, blank=True, null=True)
...
class Facture(models.Model):
    ...
    modelimp = models.CharField(verbose_name="Modèle d'impression", max_length=100, blank=True, null=True)
...
class Reglement(models.Model):
    ...
    modelimp = models.ForeignKey(ModeleImpression, verbose_name="Modèle d'impression", on_delete=models.PROTECT)
...
class PortailRenseignement(models.Model):
    ...
    idobjet = models.IntegerField(verbose_name="ID objet", blank=True, null=True)
    activite = models.ForeignKey(Activite, verbose_name="Famille", blank=True, null=True, on_delete=models.CASCADE)
