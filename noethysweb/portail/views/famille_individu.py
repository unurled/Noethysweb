...
from crispy_forms.layout import Layout, Submit, Hidden, Div, HTML, Fieldset, Field
...
class IndividuForm(forms.ModelForm):
    ...
    photo = forms.ImageField(required=False, label="Photo de l'enfant", help_text="Format JPG ou PNG recommandé")
    ...
    contact_nom = forms.CharField(required=False, label="Nom du contact", max_length=200)
    contact_prenom = forms.CharField(required=False, label="Prénom du contact", max_length=200)
    contact_lien = forms.CharField(required=False, label="Lien avec l'enfant", max_length=200, 
                                   help_text="Ex: Mère, Père, Grand-mère, Tante...")
    contact_tel_mobile = forms.CharField(required=False, label="Téléphone portable", max_length=100)
    contact_tel_domicile = forms.CharField(required=False, label="Téléphone domicile", max_length=100)
    contact_autorisation_sortie = forms.BooleanField(
        required=False, 
        initial=True,
        label="Autorisé à récupérer l'enfant"
    )
    contact_autorisation_appel = forms.BooleanField(
        required=False,
        initial=True,
        label="À contacter en cas d'urgence"
    )

    class Meta:
        model = Individu
        fields = [
            "civilite", "prenom", "nom", "date_naiss", "cp_naiss", "ville_naiss", "photo",
            "rue_resid", "cp_resid", "ville_resid",
            "regimes_alimentaires", "allergies", "allergies_detail",
            "dispmed", "dispmed_detail", "medecin"
        ]
        widgets = {
            "cp_naiss": CodePostal(attrs={"id_ville": "id_ville_naiss"}),
            "ville_naiss": Ville(attrs={"id_codepostal": "id_cp_naiss"}),
            "regimes_alimentaires": PortailSelectManyAvecPlus(attrs={
                "url_ajax": "portail_ajax_ajouter_regime_alimentaire",
                "textes": {"champ": "Nom du régime alimentaire", "ajouter": "Ajouter un régime alimentaire"}
            }),
            "allergies": PortailSelectManyAvecPlus(attrs={
                "url_ajax": "portail_ajax_ajouter_allergie",
                "textes": {"champ": "Nom de l'allergie", "ajouter": "Ajouter une allergie"}
            }),
            "dispmed": PortailSelectManyAvecPlus(attrs={
                "url_ajax": "portail_ajax_ajouter_dispmed",
                "textes": {"champ": "Nom du dispositif médical", "ajouter": "Ajouter un dispositif médical"}
            }),
        }
        labels = {
            "civilite": "Civilité",
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        if hasattr(self.fields['regimes_alimentaires'], 'queryset'):
            self.fields['regimes_alimentaires'].queryset = self.fields['regimes_alimentaires'].queryset.order_by('nom')
        ...
        if hasattr(self.fields['dispmed'], 'queryset'):
            self.fields['dispmed'].queryset = self.fields['dispmed'].queryset.order_by('nom')
        if hasattr(self.fields['medecin'], 'queryset'):
            self.fields['medecin'].queryset = self.fields['medecin'].queryset.order_by('nom', 'prenom')
        ...
        self.helper.attrs = {'enctype': 'multipart/form-data'}  # Pour l'upload de la photo
    ...
    def save_individu(self, famille: Famille):
        ...
        if contact_nom or contact_prenom:
            contact = ContactUrgence(
                ...
                autorisation_sortie=self.cleaned_data.get('contact_autorisation_sortie', True),
                autorisation_appel=self.cleaned_data.get('contact_autorisation_appel', True),
            )
            contact.save()
...
class AjouterIndividu(CustomView, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'breadcrumb' in context and context['breadcrumb']:
            context['breadcrumb'].append({'titre': 'Ajouter un enfant', 'url': '', 'code': ''})
        
        if self.request.method == 'POST':
            context['form'] = IndividuForm(self.request.POST, self.request.FILES, user=self.request.user)
        else:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.success(request, f"L'enfant {individu.Get_nom()} a été ajouté avec succès.")
